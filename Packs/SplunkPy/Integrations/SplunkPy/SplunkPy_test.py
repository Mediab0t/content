...
ENRICHED_INCIDENT = {
    'name': 'incident3',
    'rawJSON': json.dumps({
        splunk.DRILLDOWN_ENRICHMENT: ['e1', 'e2'],
        splunk.ASSET_ENRICHMENT: ['e3', 'e4'],
        splunk.IDENTITY_ENRICHMENT: ['e5', 'e6']
    })
}
...
@pytest.mark.parametrize('integration_context, incidents, output', [
    ({}, [], []),
    ({}, [ENRICHED_INCIDENT, INCIDENT_1], [ENRICHED_INCIDENT]),
    ({}, [INCIDENT_1, INCIDENT_2], [INCIDENT_2])
])
def test_store_incident_in_ic(integration_context, incidents, output, mocker):
    mocker.patch('SplunkPy.get_integration_context', return_value=integration_context)
    mocker.patch('SplunkPy.set_integration_context')
    splunk.store_incident_in_ic(incidents)
